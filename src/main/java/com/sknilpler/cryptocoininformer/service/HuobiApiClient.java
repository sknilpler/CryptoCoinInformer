...
@Service
public class HuobiApiClient implements TickerService {

    private final ApiClient apiClient;

    public HuobiApiClient(ApiClient apiClient) {
        this.apiClient = apiClient;
    }

    @Override
    public JsonNode get24HourData() throws Exception {
        return apiClient.getData(ExchangeApiUrl24H.HUOBI_24H.getUrl24H());
    }
    ...
    public List<Ticker> parseData(JsonNode data) {
        ...
        for (JsonNode coin : data.get("data")) {
            ...
            crypto.setLastPrice(coin.get("close").asDouble());
            crypto.setPriceChangePercent(((coin.get("close").asDouble() - coin.get("open").asDouble()) / coin.get("open").asDouble()) * 100);
            crypto.setVolume(coin.get("amount").asDouble());
