...
@Service
public class KuCoinApiClient implements TickerService {

    private final ApiClient apiClient;

    public KuCoinApiClient(ApiClient apiClient) {
        this.apiClient = apiClient;
    }

    @Override
    public JsonNode get24HourData() throws Exception {
        return apiClient.getData(ExchangeApiUrl24H.KUCOIN_24H.getUrl24H());
    }
    ...
    public List<Ticker> parseData(JsonNode data) {
        ...
        for (JsonNode coin : data.get("data").get("ticker")) {
            ...
            crypto.setPriceChangePercent(Double.parseDouble(coin.get("changeRate").asText()) * 100);
            crypto.setVolume(coin.get("vol").asDouble());
